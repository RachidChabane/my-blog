...
import { getRequired, getOptional, ENV_KEYS, type EnvKey } from '../src/lib/env';
...
describe('getRequired', () => {
  ...
  it('throws when key is empty string', () => {
    expect(() => getRequired('OPENROUTER_API_KEY', { OPENROUTER_API_KEY: '' })).toThrow();
  });

  it('error message includes the key name', () => {
    expect(() => getRequired('EMBEDDINGS_API_KEY', {})).toThrow('EMBEDDINGS_API_KEY');
  });

  it('error message references .env.example', () => {
    expect(() => getRequired('CLOUDFLARE_API_TOKEN', {})).toThrow('.env.example');
  });
});

describe('getOptional', () => {
  it('returns value when key is present', () => {
    expect(getOptional('CLOUDFLARE_ACCOUNT_ID', FIXTURE)).toBe('cf-account-test');
  });
  ...
  it('returns undefined when key is empty string', () => {
    expect(getOptional('OPENROUTER_API_KEY', { OPENROUTER_API_KEY: '' })).toBeUndefined();
  });
});
