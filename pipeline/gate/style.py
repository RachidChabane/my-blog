...
no-emoji + no-em-dash hard rules (``house_style_violations``, D-007), the FR-diacritics
hard rule for ``--lang fr`` (``fr_diacritic_violations``, D-007), plus a re-check of the
``style-auditor`` verdict (``parse_style_findings`` / ``style_passes`` -- only 'clean'
passes). BLOCKs on any emoji, any em-dash, a deaccented French word, an
unparseable/absent findings file, or a non-clean verdict.
...
def check_style(body_text: str, style_findings_text: str, lang: str) -> list[str]:
    """Return a list of style problems (empty == clean). Combines the deterministic
    no-emoji + no-em-dash scan over the body, the FR-diacritics scan (fr only), and
    the style-auditor verdict re-check."""
    problems = list(house_style_violations(body_text))  # no-emoji + no-em-dash (D-007)
    if lang == "fr":
        problems.extend(fr_diacritic_violations(body_text))  # FR diacritics (D-007)
    try:
...
def _cmd(run_dir: Path, lang: str) -> int:
    ...
    if body_text is not None and style_text is not None:
        ...
        # Degraded path: still run the deterministic scans over whatever body we have,
        # and name the missing findings file (the humanize pass must have run).
        if body_text is not None:
            problems.extend(house_style_violations(body_text))
            if lang == "fr":
                problems.extend(fr_diacritic_violations(body_text))
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "M-4 style gate (FR-C3): no-emoji + no-em-dash (+ FR diacritics for "
            "--lang fr) + style-auditor 'clean' re-check."
        ),
    )
