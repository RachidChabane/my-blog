...
def build_select_prompt(
    *,
    ...
    return (
        ...
        "   SOURCE INDEPENDENCE IS A HARD CONSTRAINT ON THE CHOICE: the argue stage\n"
        "   BLOCKS any chosen topic whose sources all trace to one origin (a primary\n"
        "   plus restatements/secondary coverage of it is single-origin), and a run\n"
        "   whose topics are all single-origin dies without an article. So the chosen\n"
        "   topic AND the fallback shortlist must each be genuinely independently\n"
        "   sourced (>= 2 distinct origins that each add their own evidence). If the\n"
        "   dedup-recommended topic is single-origin, pick an independently sourced\n"
        "   candidate instead, and drop single-origin candidates from the fallback\n"
        "   shortlist. If NO candidate is independently sourced, the research stage\n"
        "   should have emitted a lesson; if it did not, prefer the least single-origin\n"
        "   candidate but expect the argue gate to bite.\n"
