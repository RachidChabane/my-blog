...
def build_argue_prompt(*, repo_root: Path, run_dir: Path) -> str:
    ...
    return (
        ...
        "   LESSON DAY (the brief's chosen_topic_id starts with 'lesson-'): the thesis\n"
        "   under test is the lesson's central EXPLANATORY claim -- the framing of the\n"
        "   subject the lesson teaches as correct where most introductions go wrong. Judge\n"
        "   THAT claim by the same standard (is it true, defensible, non-trivial as a\n"
        "   pedagogical framing); 'this is educational content' is neither a pass nor a\n"
        "   fail by itself.\n"
        + "\n"
