...
24 fills ``research`` + ``select``; task 3 adds ``argue``; task 25 adds ``draft``;
task 27 adds ``publish``. ``run()`` / ``assemble_slate`` are NOT modified here —
wiring is the seam.
"""
...
def editorial_stage_descriptions(
    ...
    *,
    ...
    Returns ``{"research": ..., "select": ..., "argue": ..., "draft": ..., "publish": ...}``.
    """
    return {
